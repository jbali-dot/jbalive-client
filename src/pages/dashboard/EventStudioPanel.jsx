...
const STEPS = [
  ...
  { id: "layout", label: "Page layout & background" },
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.1fr,2.2fr,1.2fr]">
        {/* LEFT: Step summary */}
        <LeftColumn stepIndex={stepIndex} />
        ...
        <div className="space-y-4 text-sm">
          ...
          {activeStep === "layout" && <LayoutStep onChooseFromLibrary={onChooseFromLibrary} />}
    ...
    <div className="space-y-3 text-sm">
      <div className="rounded-xl border border-gray-800 bg-[#0B1020] p-3 space-y-2">
        ...
        <ul className="text-[11px] text-gray-400 space-y-1">
          <li>{stepIndex >= 0 ? "✓" : "•"} Define event brief &amp; audience</li>
          <li>{stepIndex >= 1 ? "✓" : "•"} Upload logos, banners &amp; brand colors</li>
          <li>{stepIndex >= 2 ? "✓" : "•"} Choose layout &amp; page background</li>
          <li>{stepIndex >= 3 ? "✓" : "•"} Finalise run-of-show and CTAs</li>
        </ul>
      </div>
    ...
    <div className="space-y-3 text-sm">
      <div className="rounded-xl border border-gray-800 bg-[#0B1020] p-3 space-y-2 text-xs">
        ...
        <p className="text-[10px] text-gray-500 mt-1">
          In the future this will connect to a real booking link, assign a producer and
          attach the session to this event.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-gray-800 bg-[#050814] p-3 space-y-2 text-xs">
        ...
        <p className="text-[10px] text-gray-500">
          Later this can sync to the event’s internal notes and be visible to your team
          only.
        </p>
      </div>
    </div>
...
function BrandingStep({ onChooseFromLibrary, mediaSelections }) {
  const logoInputRef = useRef(null);
  const heroInputRef = useRef(null);

  return (
    <div className="rounded-xl border border-gray-800 bg-[#0B1020] p-4 space-y-4 text-xs">
        ...
        <div className="space-y-2">
          ...
          <div className="rounded-md border border-dashed border-gray-700 bg-[#050814] p-3 space-y-2 text-[11px] text-gray-500">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                type="button"
                className="px-3 py-1.5 rounded-full bg-indigo-600 hover:bg-indigo-500 text-[11px] font-semibold"
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (file) {
                    // For now we just log it; in future this can sync to backend/media library
              ...
              <button
                type="button"
                className="px-3 py-1.5 rounded-full border border-gray-700 hover:border-indigo-500 text-[11px]"
                onClick={() => onChooseFromLibrary && onChooseFromLibrary("logo")}
              >
                Choose from library
              </button>
            </div>

            {mediaSelections?.logo && (
              <p className="text-[10px] text-emerald-300 mt-1">
                Selected from library: {mediaSelections.logo.name}
              </p>
            )}
          </div>
        </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <input
                ...
                onChange={(e) => {
                  const file = e.target.files?.[0];
                  if (file) {
                    console.log("Uploaded hero image:", file.name);
                  }
                }}
              />
              ...
              <button
                type="button"
                className="px-3 py-1.5 rounded-full border border-gray-700 hover:border-indigo-500 text-[11px]"
                onClick={() => onChooseFromLibrary && onChooseFromLibrary("hero")}
              >
                Choose from library
              </button>
            </div>

            {mediaSelections?.hero && (
              <p className="text-[10px] text-emerald-300 mt-1">
                Selected from library: {mediaSelections.hero.name}
              </p>
            )}
          </div>
        </div>
...
/* ---------- STEP: LAYOUT & BACKGROUND ---------- */

function LayoutStep({ onChooseFromLibrary }) {
  return (
    <div className="rounded-xl border border-gray-800 bg-[#0B1020] p-4 space-y-4 text-xs">
      ...
      <div className="grid gap-3 md:grid-cols-[1.7fr,1.3fr]">
        <div className="space-y-2">
            ...
            <div className="rounded-md border border-dashed border-gray-700 bg-[#050814] p-3 space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  type="button"
                  className="px-3 py-1.5 rounded-full border border-gray-700 hover:border-indigo-500 text-[11px]"
                  onClick={() => onChooseFromLibrary && onChooseFromLibrary("background")}
                >
                  Choose from library
                </button>
              </div>
